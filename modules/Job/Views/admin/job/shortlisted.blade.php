                    ...
                    <div class="container mb-4">
                        <div class="card shadow p-4">
                            <div class="d-flex justify-content-between align-items-start">
                              <div>
                                <h5><strong>{{ $row->candidateInfo->getAuthor->getDisplayName() ?? '' }}</strong> @if($row->candidateInfo->title)(<span class="text-muted">{{ $row->candidateInfo->title ?? '' }}</span>) @endif</h5>
                                    ...
                                    <div class="col-md-8">
                                        @if(!empty($skills))
                                            @foreach ($skills as $skill)
                                                <span class="px-3 py-1 rounded-pill bg-info bg-opacity-10 fw-semibold" style="color: white;font-weight: bold;">{{ $skill['name'] ?? '' }}</span>
                                            @endforeach
                                        @else
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                              ...
                              <div>
                                <a href="{{ route('job.admin.applicants.changeStatus', ['status' => 'rejected', 'id' => $row->id]) }}"><button class="btn btn-outline-danger btn-action">Not interested</button></a>
                                <a href="{{ route('job.admin.applicants.changeStatus', ['status' => 'hired', 'id' => $row->id]) }}"><button class="btn btn-outline-primary btn-action">Hire</button></a>
                            </div>
                          </div>
                    </div>
                </div>
